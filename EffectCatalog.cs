...
{
    private static Effect[] _effects = new Effect[GameConstants.Effect.nEffects];
    private static readonly object _lock = new object();

    public static Effect GetEffectById(int id)
    {
        if (id <= 0 || !BitOperations.IsPow2((uint)id))
            throw new ArgumentException(string.Format(GameConstants.ErrorMessages.InvalidIdEffect, id));
        ...
        if (idx < 0 || idx >= GameConstants.Effect.nEffects)
            throw new ArgumentOutOfRangeException(string.Format(GameConstants.ErrorMessages.InvalidIdEffect, id));
    ...
    private static Effect CreateEffectById(int id)
    {
        return id switch
        {
            ...
            //Otros efectos
            _ => throw new ArgumentException(string.Format(GameConstants.ErrorMessages.InvalidIdEffect, id))
        };
    }
}
